...
from Tortoise.tortoise.models.classifier import AudioMiniEncoderWithClassifierHead
from Tortoise.tortoise.models.diffusion_decoder import DiffusionTts
from Tortoise.tortoise.models.autoregressive import UnifiedVoice
from tqdm import tqdm

from Tortoise.tortoise.models.arch_util import TorchMelSpectrogram
from Tortoise.tortoise.models.clvp import CLVP
from Tortoise.tortoise.models.cvvp import CVVP
from Tortoise.tortoise.models.random_latent_generator import RandomLatentConverter
from Tortoise.tortoise.models.vocoder import UnivNetGenerator
from Tortoise.tortoise.utils.audio import wav_to_univnet_mel, denormalize_tacotron_mel
from Tortoise.tortoise.utils.diffusion import SpacedDiffusion, space_timesteps, get_named_beta_schedule
from Tortoise.tortoise.utils.tokenizer import VoiceBpeTokenizer
from Tortoise.tortoise.utils.wav2vec_alignment import Wav2VecAlignment
...
class TextToSpeech:
    ...
    #This prepares the settings to be used
    def prepare_settings(self, voice_samples=None, preset='standard', conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
            return_deterministic_state=False, temperature=.8, length_penalty=1, repetition_penalty=2.0, top_p=.8, max_mel_tokens=500,
            # CVVP parameters follow
            cvvp_amount=.0,
            # diffusion generation parameters follow
            cond_free=True, cond_free_k=2, diffusion_temperature=0.75,
            **hf_generate_kwargs):
        ...
        return auto_conditioning, diffusion_conditioning, auto_conds, diffuser
        
        
    #This uses the given settings to generate the audio
    def tts_with_preset(self, text, preset='standard', voice_samples=None, auto_conditioning = None, diffusion_conditioning = None, auto_conds = None, diffuser = None, word_count=2, verbose=False):#, **kwarg):
        """
        ...
        settings.update({'voice_samples': voice_samples, 
                    'auto_conditioning': auto_conditioning, 'diffusion_conditioning': diffusion_conditioning, 'auto_conds': auto_conds, 'diffuser': diffuser}) # allow overriding of preset settings with kwargs
        return self.tts(text, word_count, verbose=verbose, **settings)

    def tts(self, text, word_count, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
            return_deterministic_state=False,
            # autoregressive generation parameters follow
            num_autoregressive_samples=8, temperature=.2, length_penalty=0.55, repetition_penalty=1.0, top_p=.10, max_mel_tokens=280,
            # CVVP parameters follow
            cvvp_amount=.0,
            # diffusion generation parameters follow
            diffusion_iterations=1, cond_free=True, cond_free_k=2, diffusion_temperature=0.75,
             auto_conditioning = None, diffusion_conditioning = None, auto_conds = None, diffuser = None,
            **hf_generate_kwargs):
        """
        ...
        #print(f"{word_count} words = "+f"{max_mel_tokens} tokens")
            ...
            for b in range(best_results.shape[0]):
                ...
                for k in range(codes.shape[-1]):
                    if codes[0, k] == calm_token:
                        ...
                        ctokens = 0
                    if ctokens > 32:  # 8 tokens gives the diffusion model some "breathing room" to terminate speech.
                        latents = latents[:, :k]
                        break
